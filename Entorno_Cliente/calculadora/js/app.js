		...
		if(isNaN(valor)==false || valor=='.'){
			...
			if(signos=='/' && (entrada.innerHTML==0 || isNaN(entrada.innerHTML))){//si se intenta dividir entre 0
				...
				if(c==0){//codigo cuando se obtiene  el resultado por primera vez
					var evaluar=calculo(operaciones.innerHTML);//realizar la operacion
					...
					if(c==0){
						...
						operacion=calculo(entrada.innerHTML+signos+ultNumero);//realizar la operacion con el ulñtimo signo ingresado
					}
			...
			if(entrada.innerHTML==0){
				...
				var resultado=calculo(entrada.innerHTML);
			...
			resultado=calculo(entrada.innerHTML);
